...
export default function ListedCompany() {
  ...
  useEffect(() => {
    ...
    api
      .get("/business/getAllCompanies", {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        if (cancelled) return;
        const raw = res.data["Listed Companies:"] || [];
  ...
  const filtered = useMemo(() => {
    return companies.filter((c) => {
      ...
      // 2) status filter
      if (filters.status) {
        // Un-verified means anything not exactly "Verified"
        if (
          (filters.status === "Verified" && c.status !== "Verified") ||
          (filters.status === "Un-verified" && c.status === "Verified")
        ) {
          return false;
        }
      }
  ...
  // 1️⃣ Add this function at top of component
  const verifyBusiness = async (businessId) => {
    try {
      const token = sessionStorage.getItem("token");
      // optimistically disable button by marking loading state on that row
      setCompanies((prev) =>
        prev.map((c) =>
      ...
      if (data.status) {
        message.success(data.message || "Company verified!");
